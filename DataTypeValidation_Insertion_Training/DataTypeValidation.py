...
class dBOperation:
    ...
    def __init__(self):
        ...
        self.path = self.config['data_preparation']['training_db_dir']
        self.badFilePath = self.config['data_preparation']['bad_validated_raw_dir']
        self.goodFilePath = self.config['data_preparation']['good_validated_raw_dir']
            ...
            if c.fetchone()[0] == 1:
                ...
                for key in column_names.keys():
                    ...
                    try:
                        ...
                        conn.execute('ALTER TABLE Good_Raw_Data ADD COLUMN "{column_name}" {dataType}'.format(column_name=key, dataType=type1))
                    except:
                        conn.execute('CREATE TABLE  Good_Raw_Data ({column_name} {dataType})'.format(column_name=key, dataType=type1))
    ...
    def selectingDatafromtableintocsv(self, Database):
        ...
        fileFromDb = self.config['data_preparation']['Training_FileFromDB']
        fileName = self.config['data_preparation']['master_csv']
        ...
        try:
            ...
            csvFile = csv.writer(open(os.path.join(fileFromDb , fileName), 'w', newline=''), delimiter=',', lineterminator='\r\n', quoting=csv.QUOTE_ALL, escapechar='\\')
