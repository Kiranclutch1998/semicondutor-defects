...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self, train_x, train_y):
        ...
        try:
            ...
            self.param_grid = {"n_estimators": self.config['training']['random_forest']['param_grid']['n_estimators'], "criterion": self.config['training']['random_forest']['param_grid']['criterion'],
                               "max_depth": range(2, 4, 1), "max_features": self.config['training']['random_forest']['param_grid']['max_features']}

            # Creating an object of the Grid Search class
            self.grid = GridSearchCV(estimator=self.clf, param_grid=self.param_grid, cv=self.config['training']['random_forest']['cv'], verbose=self.config['training']['random_forest']['verbose'])
    ...
    def get_best_params_for_xgboost(self, train_x, train_y):
        ...
        try:
            ...
            self.param_grid_xgboost = {
                'learning_rate': self.config['training']['xg_boost']['param_grid']['learning_rate'],
                'max_depth': self.config['training']['xg_boost']['param_grid']['max_depth'],
                'n_estimators': self.config['training']['xg_boost']['param_grid']['n_estimators']
            }
            # Creating an object of the Grid Search class
            self.grid = GridSearchCV(XGBClassifier(objective='binary:logistic'), self.param_grid_xgboost, verbose=self.config['training']['xg_boost']['verbose'],
                                     cv=self.config['training']['xg_boost']['cv'])
            # finding the best parameters
            self.grid.fit(train_x, train_y)
