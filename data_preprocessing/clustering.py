...
class KMeansClustering:
    ...
    def elbow_plot(self, data):
        ...
        try:
            for i in range(1, self.config['data_preprocessing']['KMeansClustering']['n_cluster_max']):
                kmeans = KMeans(n_clusters=i, init=self.config['data_preprocessing']['KMeansClustering']['init'], random_state=self.config['base']['random_state'])  # initializing the KMeans object
                kmeans.fit(data)  # fitting the data to the KMeans Algorithm
                wcss.append(kmeans.inertia_)
            plt.plot(range(1, self.config['data_preprocessing']['KMeansClustering']['n_cluster_max']), wcss)  # creating the graph between WCSS and the number of clusters
            plt.title('The Elbow Method')
            plt.xlabel('Number of clusters')
            plt.ylabel('WCSS')
            # plt.show()
            plt.savefig('data/preprocessing_data/K-Means_Elbow.PNG')  # saving the elbow plot locally
            # finding the value of the optimum cluster programmatically
            self.kn = KneeLocator(range(1, self.config['data_preprocessing']['KMeansClustering']['n_cluster_max']), wcss, curve=self.config['data_preprocessing']['KMeansClustering']['KneeLocator']['curve'], direction=self.config['data_preprocessing']['KMeansClustering']['KneeLocator']['direction'])
    ...
    def create_clusters(self, data, number_of_clusters):
        ...
        self.data = data
        try:
            self.kmeans = KMeans(n_clusters=number_of_clusters, init=self.config['data_preprocessing']['KMeansClustering']['init'], random_state=self.config['base']['random_state'])
